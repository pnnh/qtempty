//
// Created by Larry on 2021/11/15.
//
...
int randRgb() {
  return QRandomGenerator::global()->bounded(255) % 255;
}
...
void ColorMaker::timerEvent(QTimerEvent *e) {
  if (e->timerId() == m_nColorTimer) {
    switch(m_algorithm) {
      case RandomRGB:
        m_currentColor.setRgb(randRgb(), randRgb(), randRgb());
        break;
      case RandomRed:
        m_currentColor.setRed(randRgb());
        break;
      case RandomGreen:
        m_currentColor.setGreen(randRgb());
        break;
      case RandomBlue:
        m_currentColor.setBlue(randRgb());
        break;
      case LinearIncrease:
        {
